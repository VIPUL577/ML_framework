...
namespace seera
{
    ...
    void conv2d_forward(const float *X, const float *W, float *out,
                        ...
                        int F, int KH, int KW, int strideh, int stridew, int padh, int padw)
    {
        int OH = (H + 2 * padh - KH) / strideh + 1;
        int OW = (Win + 2 * padw - KW) / stridew + 1;
        int col_row = C * KH * KW;
        int col_col = OH * OW;

        // Allocate im2col buffer
        float *col = new float[N * col_row * col_col];
        im2col_batch(X, col, N, C, H, Win, KH, KW, strideh, stridew, padh, padw);

// For each sample: out[n] = W_flat(F, col_row) @ col[n](col_row, col_col) → (F, col_col)
        #pragma omp parallel for schedule(static) // -> for batch in parallel
        for (int n = 0; n < N; n++)
        {
            const float *cn = col + n * col_row * col_col;
                         ...
                         int F, int KH, int KW, int strideh, int stridew, int padh, int padw)
    {
        ...
        col2im_batch(dX_col, dX, N, C, H, Win, KH, KW, strideh, stridew, padh, padw);
                                  ...
                                  int strideh, int stridew, int padh, int padw)
    {
        ...
        col2im_batch(col, out, N, Cout, Hout, Wout, KH, KW, strideh, stridew, padh, padw);
                                   ...
                                   int strideh, int stridew, int padh, int padw)
    {
        ...
        im2col_batch(dout, col_dout, N, Cout, Hout, Wout, KH, KW, strideh, stridew, padh, padw);
