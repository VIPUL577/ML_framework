...
// this whole will take 3-4 hrs, please do it. intern depends on it. 8-12 we will do it.
...
namespace seera
{
    ...
    void col2im_batch(const float *col, float *X,
                      int N, int C, int H, int W,
                      int KH, int KW, int strideh, int stridew, int padh, int padw); //-> GPU supported!!

    void conv2d_forward(const float *X, const float *W, float *out,
                        int N, int C, int H, int Win,
                        int F, int KH, int KW, int strideh, int stridew, int padh, int padw); //-> GPU supported!!

    void conv2d_backward(const float *dout, const float *X, const float *W,
                         float *dX, float *dW,
                         int N, int C, int H, int Win,
                         int F, int KH, int KW, int strideh, int stridew, int padh, int padw); // wrapper to be written
    ...
    void maxpool2d_backward(const float *dout, const int32_t *mask, float *dX,
                            ...
                            int strideh, int stridew, int padh, int padw); //-> GPU supported!

    // ── Unpooling Nearest ─────────────────────────────────────
    void unpooling_fwd(const float *x, float *out,
                       int N, int C, int H, int W, int sh, int sw); //-> GPU supported!
    ...
    void conv_transpose2d_forward(const float *X, const float *W, float *out,
                                  ...
                                  int strideh, int stridew, int padh, int padw); // wrapper likhna padgea

    void conv_transpose2d_backward(const float *dout, const float *X, const float *W,
                                   float *dX, float *dW,
                                   int N, int Cin, int H, int Win,
                                   int Cout, int KH, int KW,
                                   int strideh, int stridew, int padh, int padw); // wrapper likhan padega
